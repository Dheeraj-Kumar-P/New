class MaidController < ApplicationController 
	...
	def task
		...
		(3...length).each do |iteration|
			TaskAssignment.create(:user_id=>params[:users][:id],:room_id=>params[params.keys[iteration]],:task_id=>'1',:status=>"assigned")
		end
	...
	def stop
		...
		if(!@salary.total_hours.nil?)	#date and time is being added with that in salary
			date=@salary.date
